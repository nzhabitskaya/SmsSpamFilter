...
public class AllowSmsAdapter extends ArrayAdapter<BlockItem> {
	private Context mContext;
	private RegexpDialog regexpDialog;
	private UpdateDialog updateDialog;
	private BlockedListWrapper allowedListWrapper;

	public AllowSmsAdapter(Context context, int textViewResourceId) {
		super(context, textViewResourceId);
		mContext = context;
		regexpDialog = new RegexpDialog(context, this, DataProvider.ALLOWED_CONTENT_URI);
		updateDialog = new UpdateDialog(context, this, DataProvider.ALLOWED_CONTENT_URI);
		
		/** Use wrapper to DB access */
        allowedListWrapper = new BlockedListWrapper(context, DataProvider.ALLOWED_CONTENT_URI);
	}
	
	public int getCount(){
		return allowedListWrapper.getCount();
	}
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (item != null) {
			...
			if(typeId==0){
				...
				textView.setText(mContext.getString(typeId) + ": " + (address!=null?(address):from));
			...
			cb.setOnClickListener(new OnClickListener() {				
				...
				public void onClick(View v) {
					...
					allowedListWrapper.updateBlockedItem(item.getId(), cb.isChecked());
				}
			});
